...
bool EMF::Walkable(unsigned char x, unsigned char y, bool npc, bool noWarp) const
{
    if (!InBounds(x, y) || !this->GetTile(x, y).Walkable(npc, noWarp))
    {
        return false;
    }
...
void EIF::Read(const std::string& filename)
{
    ...
    printf("%i items loaded. - ", (int)this->data.size()-1);
...
void ENF::Read(const std::string& filename)
{
    ...
    printf("%i npc types loaded. - ", (int)this->data.size()-1);
...
void ESF::Read(const std::string& filename)
{
    ...
    printf("%i spells loaded. - ", (int)this->data.size()-1);
...
void ECF::Read(const std::string& filename)
{
    ...
    printf("%i classes loaded. - ", (int)this->data.size()-1);
